...
class APIClient:
    ...
    def _authenticate(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not token:
                    raise ValueError(
                        f"Authentication response missing token field. Response: {data}"
                    )
            ...
            raise ValueError(f"Authentication request failed: {e}") from e
    ...
    def post_videos(self, videos: List[Video]) -> Dict[str, Any]:
        ...
        results = {"success": True, "posted": 0, "failed": 0, "errors": []}
            ...
            try:
                ...
                if response.get("success"):
                    ...
                    results["errors"].append(
                        {"video_id": video.video_id, "error": error_msg}
                    )
    ...
    def _log_video_details(self, video: Video) -> None:
        ...
        logger.info(f"   Channel ID: {video.channel_id}")
        logger.info(f"   Channel URL: {channel_url}")
        logger.info("\n   📊 COMPLETE DATA (SENDING TO API):")
        logger.info(
            f"   ├─ Category: {video.category_name or 'N/A'} (ID: {video.category_id or 'N/A'})"
        )
        ...
        logger.info(
            f"   ├─ Duration: {video.duration_seconds} seconds ({video.duration_iso})"
        )
            ...
            while True:
                ...
                if not response.ok:
                    if response.status_code in [401, 403]:
                        logger.warning(
                            "⚠️  AUTHENTICATION ERROR: Check if API token is correct"
                        )
                    logger.warning(
                ...
                if total_pages is None:
                    ...
                    logger.info(
                        f"📊 Total items: {total_items}, Total pages: {total_pages}"
                    )
                ...
                logger.info(
                    f"  ✓ Page {current_page + 1}/{total_pages}: {len(page_urls)} URLs fetched"
                )
